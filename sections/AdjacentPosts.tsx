...
const AdjacentPosts: React.FC<AdjacentPostProps> = ({ createdAt, slug }) => {
  const [adjacentPost, setAdjacentPost] = useState<AdjacentPostInterface | null>(null);
  const [dataLoaded, setDataLoaded] = useState(false);

  useEffect(() => {
    const postService = new PostService();
    postService.getAdjacentPosts(createdAt, slug).then((result) => {
